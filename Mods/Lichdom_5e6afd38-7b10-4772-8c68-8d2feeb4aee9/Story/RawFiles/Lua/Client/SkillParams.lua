...
local function GetSoulReaperDescription(skill, character, isFromItem, param)
	if ClientData["SoulReaper"] ~= nil then
		...
		if data ~= nil then
			local description = Ext.GetTranslatedStringFromKey("Shout_LLLICH_SoulReaper_Description_Enabled")
			...
			local description = Ext.GetTranslatedStringFromKey("Shout_LLLICH_SoulReaper_Description_Disabled")
			return description
		end
	end
	return ""
